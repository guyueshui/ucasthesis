...
在信息论中，熵（Entropy）是信息量的度量，相对熵（Relative Entropy）和互信息（Mutual Information）均为两个分布之间的关系度量，它们的定义如下\citep{cover2012elements}：
\begin{definition}{熵}
...
在信息论中，熵是随机变量不确定性的度量，用来衡量它所持有的信息量。由以上定义，容易得到
\begin{equation}
  \begin{split}
    I(X;~Y) &= H(X) - H(X|Y) \\
    &= H(Y) - H(Y|X).
